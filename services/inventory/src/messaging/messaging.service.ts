...
import { EventPublisher } from '../inventory/application/ports/event-publisher';
...
/**
 * Cliente RabbitMQ (amqplib) con reconexión. Expone un exchange `topic`
 * durable `optimus.events`. Implementa EventPublisher (publish) y permite
 * registrar suscripciones que se re(establecen) al conectar.
 */
@Injectable()
export class MessagingService implements EventPublisher, OnModuleInit, OnModuleDestroy {
  ...
  /** Registra una suscripción; se establece ya si hay canal, o al (re)conectar. */
