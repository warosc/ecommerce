...
/**
 * Cliente RabbitMQ (amqplib) con reconexión, solo de consumo para el CRM.
 * Se suscribe al exchange `topic` durable `optimus.events`.
 */
@Injectable()
export class MessagingService implements OnModuleInit, OnModuleDestroy {
  ...
  private async connectWithRetry(attempt = 1): Promise<void> {
    try {
      ...
      // Un mensaje en vuelo a la vez: procesa los eventos en serie y evita el
      // lost-update al acumular el perfil del mismo cliente (read-modify-write).
